...
with versions as (
    select
        ...
        md5(concat_ws('|', coalesce(NATIONALITY,''), coalesce(ROLE,''), coalesce(BATTING_STYLE,''), coalesce(BOWLING_STYLE,''), coalesce(CURRENT_TEAM_ID,''))) as HASH_DIFF
...
from versions
union all
select 0, 'UNKNOWN', 'Unknown', '', '', '', '', '', 'UNKNOWN', 'UNKNOWN', '1900-01-01'::timestamp_ntz, '9999-12-31'::timestamp_ntz, true, md5('UNKNOWN')
